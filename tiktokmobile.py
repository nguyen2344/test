...
soacc=lanclick =biendem = tongxu = solantim = die_job_tiktok=binhluan = 0
...
def aut():
    ...
    if os.path.exists(filename):
        ...
        token = input("Nhập token Bearer: ").strip()
        with open("authorization.txt", "w") as f:
            f.write(token)  # ✅ sửa chỗ này
        aut = token        # ✅ gán giá trị để return
    return aut
...
def user_info(headers):
    ...
    response = scraper.get('https://gateway.golike.net/api/users/me', headers=headers)
    user = response.json()
    banner()
    print(f"{MAGENTA}Tên{RESET}: {YELLOW}{user['data']['name']}{RESET}")
    print(f"{MAGENTA}Xu{RESET}: {YELLOW}{user['data']['coin']}{RESET}")
...
def get_nick(headers):
    global soacc
    headers = build_headers(aut)
    response = scraper.get('https://gateway.golike.net/api/tiktok-account', headers=headers).json()
    accounts = []
    soacc = 0  # nên khởi tạo lại để tránh cộng dồn nếu gọi nhiều lần
    ...
    while True:
        ...
        if 0 <= index < len(accounts):
            ...
            print("Số nhập không hợp lệ! Vui lòng chọn lại.", end="\r")
def get_job(headers, selected_acc, second):
    global lanclick, biendem, tongxu, session, solantim, die_job_tiktok, binhluan ,scraper
    os.system("cls")
    banner()
    while True:
        try:
            job = []
            headers = build_headers(aut)
            account_id = selected_acc['acc_id']
